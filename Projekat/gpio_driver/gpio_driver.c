...
//isto iz infrared
/*static int sensor = 1;
module_param(sensor, int, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(sensor, "An integer value of SENSOR.");*/
...
static ssize_t gpio_driver_read(struct file *filp, char *buf, size_t len, loff_t *f_pos)
{
    ...
    gpio_driver_buffer[data_size++] = GetGpioPinValue(GPIO_17);
		
	
    
    /*if(GetGpioPinValue(GPIO_17) == 1)
    {
        sprintf(gpio_driver_buffer, "%d", speed);
        return 1;
    }*/
